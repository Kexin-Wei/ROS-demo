...
class TurtleControllerNode : public rclcpp::Node {
 public:
  TurtleControllerNode()
      : Node("turtle_controller"), name_("turtle1"), turtlesim_up_(false) {
    this->declare_parameter("catch_closest_turtle_first", true);
    catch_closest_turtle_first_ =
    ...
    pose_subscriber_ = this->create_subscription<turtlesim::msg::Pose>(
        ...
        std::bind(&TurtleControllerNode::callbackPose, this,
                  std::placeholders::_1));
    turtles_to_catch_subscriber_ =
        this->create_subscription<my_robot_interfaces::msg::TurtleArray>(
            "alive_turtles", 10,
            std::bind(&TurtleControllerNode::callbackTurtlesToCatch, this,
                      std::placeholders::_1));
 ...
 private:
  ...
  void callbackTurtlesToCatch(
      const my_robot_interfaces::msg::TurtleArray::SharedPtr msg) {
    if (!msg->turtles.empty()) {
      if (catch_closest_turtle_first_) {
        my_robot_interfaces::msg::Turtle closest_turtle = msg->turtles.at(0);
        double closest_turtle_distance =
            getDistanceFromCurrentPose(closest_turtle);
  ...
  void controlLoop() {
    ...
    if (distance > 0.5) {
      ...
      catch_turtle_threads.push_back(std::make_shared<std::thread>(
          std::bind(&TurtleControllerNode::callCatchTurtleService, this,
  ...
  void callCatchTurtleService(std::string turtle_name) {
    auto client = this->create_client<my_robot_interfaces::srv::CatchTurtle>(
        "catch_turtle");
    ...
    auto request =
        std::make_shared<my_robot_interfaces::srv::CatchTurtle::Request>();
  ...
  rclcpp::Subscription<my_robot_interfaces::msg::TurtleArray>::SharedPtr
      turtles_to_catch_subscriber_;
